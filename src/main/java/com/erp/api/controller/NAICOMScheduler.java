...
import com.fasterxml.jackson.databind.ObjectMapper;
	...
	EngineeringClaimDB engineeringClaimDB;
		...
		public void NAICOMScheduler() {
			
			ResponseMessage responseMessage1=new ResponseMessage();
			List<PolicyUpdateDetailsDto> array=new ArrayList<>();
			responseMessage1=engineeringClaimDB.getNAICOMPendingRecords(responseMessage1);
		...
		String response1=restTemplateUtil.callPostAPIwithToken(json, updatePolicyUrl, policyUpdateDetailsDto.getToken());
		if(response1!=null) {
		try {
		...
		public void sendWebhook(PolicyUpdateDetailsDto policyUpdateDetailsDto,ResponseMessage response,Object data) {	
			log.info("request is inside sending webhook for Update Naicom Policy");
		...
		responseJson.put("policyNumber", policyUpdateDetailsDto.getPolicyNumber());
		if(dt.has("policyCertificate"))
		responseJson.put("policyCertificate", dt.getString("policyCertificate"));
		//if(dt.has("POLICY_ID"))
		responseJson.put("policyId", policyUpdateDetailsDto.getPolicyId());
		if(dt.has("policyDocument"))
		responseJson.put("policyDocument", dt.getString("policyDocument"));
		...
		if(policyUpdateDetailsDto.getWebHookUrl()!=null) {
			...
			try {
				...
				String responseData=restTemplateUtil.callBackAPIwithHeader(creditResponse.toString(),policyUpdateDetailsDto.getWebHookUrl(),policyUpdateDetailsDto.getToken());
		...
		log.info("response is sent for webhook for update Naicom policy");
		}
